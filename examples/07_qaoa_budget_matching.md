...
（`c_i` はコスト、`x_i ∈ {0,1}` は選ぶかどうか）。

```
minimize:  (Σ c_i x_i − budget)^2
```

を展開すると `x_i^2 = x_i`（0/1変数の性質）を使って、線形項と2次項だけのQUBOに落とし込めます。

```
coeff(x_i)      = c_i^2 − 2 * budget * c_i
coeff(x_i, x_j) = 2 * c_i * c_j
```
...
理論上の最小値 `-budget^2 = -100` を達成しています（この定式化では定数項 `budget^2` を省いているので、
QUBO値`-100`は「誤差0でぴったり一致」を意味します）。
...
- 完全一致する組み合わせが存在しない場合は、QUBO値が最小でも0にはなりません。「最も予算に近い組み合わせ」
  として使えますが、必ず`(Σc_i x_i - budget)`を自分で計算して許容誤差内かを確認してください。
