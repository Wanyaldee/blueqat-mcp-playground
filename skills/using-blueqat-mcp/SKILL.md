...
|---|---|---|
| Pick the best subset/combination (routing, portfolio, budget matching, scheduling, frequency/graph coloring, MaxCut-style pairing, ML feature selection) | Encode as a QUBO (minimize form) → `run_qaoa` | Start `steps`/`p` at 1-2; raise `n_starts` only if results look unstable (both are tier-capped — see below). Always pass or record `seed` for reproducibility. The free tier also caps the **qubo itself at 20 terms total** (linear + quadratic combined) — not listed in `sdk_info`, only surfaces as `CircuitBuildError: too many qubo terms`. A dense pairwise term (e.g. a full cardinality-constraint penalty over n candidates needs C(n,2) quadratic terms) blows past this fast; prune to the terms that matter (e.g. only genuinely-redundant pairs above a real correlation threshold) rather than including every pair. |
...
| Unstructured search with a known "is this the answer?" test, incl. toy key-search / cryptanalysis demos | `run_circuit` with a hand-built oracle (no dedicated Grover tool exists) | Quadratic speedup is asymptotic; a small toy circuit only demonstrates the algorithm, not a wall-clock win. `list_gates` only offers 2-control gates (`ccx`/`ccz`) — marking a target wider than 3 qubits needs an ancilla-based multi-control decomposition, not a direct gate. **Never present a toy-cipher key search as an attack on a real cipher** (AES etc. would need thousands of logical qubits to even express as a reversible circuit) — say explicitly that it demonstrates the search mechanic only. |
...
| Eavesdropping-detectable key exchange (QKD, e.g. BB84) | `run_circuit`: encode each trial as a signal+ancilla qubit pair; simulate an eavesdropper by rotating into their basis, `cx`(signal→ancilla), rotating back — entangling with an ancilla you never read reproduces measurement backaction without needing mid-circuit measurement | The advantage here is *detectability of eavesdropping*, not speed. **State clearly that a simulated run proves nothing about real secure key exchange** — there is no physical quantum channel between the simulated parties, so the output must never be treated as an actual shared secret. |
